...
describe('mounted Calculator', () => {
	...
	it('calls updateDisplay when a number key is clicked', () => {
		// spy for the calculator method being tested
		const spy = jest.spyOn(wrapper.instance(), 'updateDisplay');
		wrapper.instance().forceUpdate(); //call forceUpdate to re-render the instance within the test
		expect(spy).toHaveBeenCalledTimes(0); //number of times function is being called
		wrapper.find('.number-key').first().simulate('click'); //simulate click of button
		expect(spy).toHaveBeenCalledTimes(1); //number of times function is being called
	});
	...
	it('calls setOperator when an operator key is clicked', () => {
		...
		wrapper.instance().forceUpdate(); //call forceUpdate to re-render the instance within the test
		expect(spy).toHaveBeenCalledTimes(0); //number of times function is being called
		wrapper.find('.operator-key').first().simulate('click'); //simulate click of button
		expect(spy).toHaveBeenCalledTimes(1); //number of times function is being called
	});
	...
	it('calls callOperator when the submit key is clicked', () => {
		...
		wrapper.instance().forceUpdate(); //call forceUpdate to re-render the instance within the test
		expect(spy).toHaveBeenCalledTimes(0); //number of times function is being called
		wrapper.find('.submit-key').simulate('click'); //simulate click of button
		expect(spy).toHaveBeenCalledTimes(1); //number of times function is being called
	});
});
